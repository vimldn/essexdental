...
export default function ArticlePage() {
    ...
    return (
      <div className="min-h-screen bg-white text-slate-700">
        ...
        <div className="pt-32 px-6 max-w-5xl mx-auto">
          ...
          <Link href="/blog" className="text-emerald-500 underline mt-6 inline-block">Back to blog</Link>
        </div>
        <Footer />
      </div>
    );
  ...
  const articleClasses = [
    'p-10 max-w-none',
    '[&_h1]:text-4xl [&_h1]:md:text-5xl [&_h1]:font-black [&_h1]:tracking-tight [&_h1]:text-slate-900 [&_h1]:mt-10 [&_h1]:mb-5',
    '[&_h2]:text-3xl [&_h2]:md:text-4xl [&_h2]:font-black [&_h2]:tracking-tight [&_h2]:text-slate-900 [&_h2]:mt-10 [&_h2]:mb-4',
    ...
    '[&_a]:text-emerald-500 [&_a]:font-black [&_a]:underline [&_a]:underline-offset-4 hover:[&_a]:text-emerald-600',
    ...
    '[&_li]:leading-relaxed [&_li]:pl-1 [&_li]:marker:text-emerald-500',
            ...
            <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-10">
              {relatedArticles.map((a) => (
                <Link key={a.Slug} href={`/blog/${a.Slug}`}
                  className="group rounded-[2.5rem] border border-slate-200 overflow-hidden flex flex-col hover:border-emerald-300 transition-all duration-500 shadow-sm hover:shadow-lg bg-white">
                  <div className="relative h-44 overflow-hidden bg-gradient-to-br from-slate-100 to-slate-50">
                    ...
                    <div className="absolute top-5 left-5 px-4 py-1.5 bg-emerald-500 text-white text-[10px] font-black uppercase rounded-full">
                      {a.wp_category}
                    </div>
                  </div>
                  <div className="p-8 flex-1 flex flex-col">
                    <h3 className="text-xl font-black text-slate-900 mb-4 group-hover:text-emerald-500 transition-colors">
                      {a['Article Title']}
                    </h3>
                    <div className="flex items-center gap-2 text-emerald-500 font-black uppercase tracking-widest text-[10px] mt-auto">
              ...
              {furtherReading.map((l) => (
                <li key={l.url}>
                  <a href={l.url} target="_blank" rel="noopener noreferrer"
                    className="text-emerald-500 underline underline-offset-4 hover:text-emerald-600">{l.label}</a>
                </li>
              ))}
            </ul>
          </div>
